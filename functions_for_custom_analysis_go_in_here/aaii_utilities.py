...
def return_percentile_rank(stock_list, attribute_name_str, stock_symbol=None, reverse=False):
    ...
    for this_stock in stock_list:
        try:
            ...
            if error_count < 10:
                print(exception, "Stock appears to have no %s attribute." % attribute_name_str)
            else:
                print(exception, "Multiple stocks appears to have no %s attribute." % attribute_name_str)
                return
            error_count+=1
