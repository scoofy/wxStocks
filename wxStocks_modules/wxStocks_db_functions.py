...
named_screen_path = 'wxStocks_data/screen-{}.pk'
...
portfolios_path = 'DO_NOT_COPY/portfolios.{}'
portfolio_account_obj_file_path = 'DO_NOT_COPY/portfolio_{}_data.{}'
...
def load_GLOBAL_STOCK_DICT():
    ...
    try:
        ...
        logging.warning("If this is your first time opening wxStocks, please ignore the following exception, otherwise, your previously saved data may have been deleted.\n{}".format(e))
...
def load_named_screen(screen_name):
    ...
    try:
        screen_file = open(named_screen_path.format(screen_name.replace(' ','_'), 'rb'))
        screen_ticker_list = pickle.load(screen_file)
        screen_file.close()
    except Exception as e:
        logging.error("Screen: {} failed to load.\n{}".format(screen_name, e))
    stock_list = []
...
def save_named_screen(screen_name, stock_list):
    ...
    with open(named_screen_path.format(screen_name.replace(' ','_'), 'wb')) as output:
        pickle.dump(ticker_list, output, pickle.HIGHEST_PROTOCOL)
def delete_named_screen(screen_name):
    ...
    os.remove(named_screen_path.format(screen_name.replace(' ', '_')))
...
def decrypt_if_possible(path):
    ...
    if config.ENCRYPTION_POSSIBLE:
        ...
        try:
            ...
            if e.errno == 2:
                logging.error("Decryption not possible, account file {} doesn't exist. This is likely not a problem, unless you use that portfolio regularly".format(path))
            else:
                logging.error(e)
            encrypted_file = None
        if encrypted_file:
            try:
                ...
                logging.error("encryption string or close error\n{}".format(e))
            ...
            if not encrypted_string:
                ...
                if not pickled_string:
                    ...
                    try:
                        ...
                        logging.error("depickle error: Decryption not possible, account file doesn't exist\n{}".format(e))
        fernet_obj = None
        if not data:
            try:
                ...
                logging.warning("Decryption not possible, but unencrypted file exists and will be used instead.")
            except Exception as e:
                #print line_number(), e
                error = True
    else:
    ...
    if not error:
        ...
        if config.ENCRYPTION_POSSIBLE:
            #logging.error("Error loading encrypted file: {}".format(path))
            pass
        else:
            #logging.error("Error loading unencrypted file: {}".format(path))
            pass
        return None
...
def save_portfolio_object(portfolio_obj):
    ...
    if config.ENCRYPTION_POSSIBLE:
        fernet_obj = Fernet(config.PASSWORD)
        path = portfolio_account_obj_file_path.format(id_number, "txt")
        ...
        path = portfolio_account_obj_file_path.format(id_number, "pk")
        with open(path, 'wb') as output:
...
def load_all_portfolio_objects():
    ...
    logging.info("attempting to load {} possible portfolios".format(total_num_of_possible_account_files))
...
def delete_portfolio_object(id_number):
    ...
    if portfolio_obj:
        try:
            ...
            logging.error(e)
            if config.ENCRYPTION_POSSIBLE:
                logging.info("Deleting encrypted file failed, will attempt to delete an unencrypted instance of the file.")
                try:
                    os.remove(path.replace(".txt", ".pk"))
                except Exception as e:
                    logging.error(e)
                    logging.info("Error: account object failed to be deleted.")
                    return
            else:
                logging.info("Error: account object failed to be deleted.")
                return
        portfolio_obj = None
        logging.info("Account object has been deleted.")
        return True
    else:
        logging.error("Error: account object failed to be deleted. It may not exist.")
        return False
