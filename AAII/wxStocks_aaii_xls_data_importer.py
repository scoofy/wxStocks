import sys, pprint, os, inspect, string, time, logging, config, threading
...
def process_aaii_thread_function(aaii_filenames, data_directory):
...
def import_aaii_files_from_data_folder(path, time_until_data_needs_update = 999604800): # one week 604800
    ...
    aaii_filenames.sort()
    # logging.info([os.path.join(data_directory, filename) for filename in aaii_filenames])
    # logging.info(config.SET_OF_FILENAMES_OF_IMPORTED_FILES)
    previously_processed_filenames = [filename for filename in aaii_filenames
                                        if os.path.join(data_directory, filename) in config.SET_OF_FILENAMES_OF_IMPORTED_FILES]
    if previously_processed_filenames:
        logging.info("These files have already been imported: {}".format([os.path.join(data_directory, filename) for filename in previously_processed_filenames]))
        # remove duplicates
        aaii_filenames = [filename for filename in aaii_filenames
                            if filename not in previously_processed_filenames]
    logging.info("Importing the following files: {}".format(aaii_filenames))
    ...
    aaii_thread = threading.Thread(target=process_aaii_thread_function, args=(aaii_filenames, data_directory))
    aaii_thread.start()
...
def process_aaii_xls_data_file(filename, key_dict, the_files_index_in_file_list, number_of_files_being_processed):
    with db.db.transaction() as connection:
        ...
        for row_list in spreadsheet_list_of_row_data_list:
            ...
            for attribute in spreadsheet_list_of_row_data_attributes:
                ...
                db.set_Stock_attribute(current_stock, long_attribute_name, datum, "_aa", connection=connection)
    ...
    logging.info("Finished importing {}".format(filename))
    config.SET_OF_FILENAMES_OF_IMPORTED_FILES.add(filename)
